...
def app():
    ...
    online_order = st.selectbox('online ordering is available in the restaurant or not', ['Yes', 'No'])
    book_table	 = st.selectbox('table book option available or not', ['Yes', 'No'])
    ...
    if predict:
        ...
        if pred == 1:
            st.write(F"Prediction: Good Resturant ")

        else:
            st.write(F"Prediction: Bad Resturant ")         
